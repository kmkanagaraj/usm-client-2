...
export class ImportClusterController {
        ...
        this.clusterService.getImportClusterDeatils(this.bootstrapNode.hostname).then((result) => {
            if (result.status === 200) {
                ...
                if (!this.clusterSummary.compatible) {
                    this.error = "Cluster is not compatible . Please ensure that Ceph 2.0 or greater is installed on this host. Click Refresh to search for this host again."
    ...
    private isAllHostsFound(clusterSummary: any): boolean {
        return _.every(clusterSummary.nodes, (node: any) => node.found === true);
    }
