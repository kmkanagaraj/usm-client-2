...
export class ImportClusterController {
    ...
    public loadClusterDetails(): void {
        ...
        this.clusterService.getImportClusterDeatils(this.bootstrabNode.hostname).then((result) => {
            if (result.status === 200) {
                ...
                this.error = "Failed to retrive cluster information from the selected host '" + this.bootstrabNode.hostname + "'. Please select a monitor host and try again";
            }
        }, (error) => {
            this.error = "Failed to retrive cluster information from the selected host '" + this.bootstrabNode.hostname + "'. Please select a monitor host and try again";
        });
    ...
    public import(): void {
        var cluster = { 'bootstrapnode': this.bootstrabNode.hostname, 'clusterType': 'ceph', 'nodes': this.clusterSummary.nodes };
        this.clusterService.importCephCluster(cluster).then((result) => {
            if (result.status === 202) {
                ...
                var modal = ModalHelper.SuccessfulRequest(this.modalService, {
                    title: 'Import Cluster request is submitted',
