...
export class ClusterDetailController {
    ...
    public getOpenStack() {
        this.openStack.push({"title":"Cinder","units":"GB","data":{"used":"25","total":"100"}});
        this.openStack.push({"title":"Cinder-Backup","units":"GB","data":{"used":"75","total":"100"}});
        this.openStack.push({"title":"Glance","units":"GB","data":{"used":"86","total":"100"}});
        this.openStack.push({"title":"Nova","units":"GB","data":{"used":"30","total":"100"}});
    }
    ...
    public getClusterUtilization(utilizations: Array<any>) {
        _.each(utilizations, (utilization: any) => {
            if (utilization.target.split('.')[3] === 'total_bytes') {
                this.capacity.total = numeral(utilization.datapoints[0][0]).format('0 b');
                this.clusterUtilization.data.total = utilization.datapoints[0][0];
            }else if (utilization.target.split('.')[3] === 'total_used_bytes') {
                this.capacity.used = numeral(utilization.datapoints[0][0]).format('0 b');
                this.clusterUtilization.data.used = utilization.datapoints[0][0];
            }
        });
        ...
        this.clusterUtilization.config.onClickFn = function (d, i) {
              alert("You Clicked On The Donut!" + d.name);
        };
    }
