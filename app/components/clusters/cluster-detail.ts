...
export class ClusterDetailController {
    ...
    constructor(private qService: ng.IQService,
        ...
        this.filterList.OSDStatus = [
            ...
            {name: "Down", icon: "fa-arrow-circle-o-down down-color"}
        ];
    ...
    public osdActionChange = () => {
        if(this.osdAction !== 'action') {
            if(this.utils.keys(this.selection.allSelectedOsds).length === 0 ) {
                this.clusterService.slusAction(this.id,this.selection.activeOsd.sluid,this.osdAction).then((response) => this.selection.allSelectedOsds= {});
            }else {
                _.forOwn(this.selection.allSelectedOsds, (value, key) => {
                    this.clusterService.slusAction(this.id,key,this.osdAction).then((response) => this.selection.allSelectedOsds= {});
                });
            }
        }
    }
