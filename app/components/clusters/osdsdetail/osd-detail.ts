...
export class OsdDetailController {
    ...
    public osdActionChange() {
        if(this.osdAction !== 'action') {
            if(this.osdAction === '{"in":false}') {
                var modal = ModalHelpers.OsdActionConfirmation(this.$modal, {}, 'Are you sure you want to move the selected OSDs to Down?');
                modal.$scope.$hide = _.wrap(modal.$scope.$hide, ($hide, confirmed: boolean) => {
                    if (confirmed) {
                           this.performOsdAction();
                    }
                    $hide();
                });
            }else {
