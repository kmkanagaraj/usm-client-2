...
export class ServerService {
    ...
    public getCpuUtilization(nodeId) {
        return this.rest.one('nodes', nodeId).one('utilization').get({ resource: 'cpu', duration: '1m' });
    }

    public getMemoryUtilization(nodeId) {
        return this.rest.one('nodes', nodeId).one('utilization').get({ resource: 'memory', duration: '1m' });
    }
}
